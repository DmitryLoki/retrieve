...
*/
		...
		this.get = function(query) {
			var self = this;
			// загрузка пилотов
			if (query.type == "pilots") {
				$.ajax({
					url: "http://dev.airtribune.com/cd5/event1120/_design/5309/_list/init/init",
					dataType: "json",
					success: function(result) {
						var data = [];
						for (var i = 0; i < result.models.length; i++) {
							var rw = result.models[i];
							data.push($.extend(rw,{
							...
							// хак: пока нет даных о race, будем выбирать сначала пилотов и из них определять startKey и endKey трека
							if (!self.options.startKey || self.options.startKey > rw.first_time)
								self.options.startKey = rw.first_time;
							if (!self.options.endKey || self.options.endKey < rw.last_time)
								self.options.endKey = rw.last_time;
						}
						self.options.startKey *= 1000;
						self.options.endKey *= 1000;
